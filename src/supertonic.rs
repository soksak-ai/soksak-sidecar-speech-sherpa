// supertonic 파이프라인 — supertone-inc/supertonic 공식 Rust 레퍼런스(MIT)를 벤더링.
// 원본: https://github.com/supertone-inc/supertonic rust/src/helper.rs (수정 최소 — 업스트림 추종 용이).
// 수정: write_wav_file(hound 의존)·timer(콘솔 출력) 제거, 나머지 원형 유지.
...
pub fn load_voice_style(voice_style_paths: &[String], verbose: bool) -> Result<Style> {
    ...
    if verbose {
        eprintln!("Loaded {} voice styles", bsz);
    }
